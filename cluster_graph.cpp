...
long double kruskal(vector<edge>&edges,const int& point_count,const int& cluster_target){
    bool finding_d = (point_count==cluster_target);
    long double d = 0;
    ...
    for(const edge& e:edges){
        if(find(e.from) != find(e.to)){
            if(finding_d){
                d = length(e.from,e.to);
                break;
            }
            else{
                union_point(e.from,e.to);
                cluster_count--;
                if(cluster_count == cluster_target){finding_d = true;}
            }
        }
    }
    return d;
...
int main(){
    ...
    cout<<kruskal(all_edges,point_count,k);
}