...
void print_cycle(const list<int>&path){
    ...
    for(auto it = path.begin(); it != prev(path.end()); it++){
        //we want to only print the last bit 
        //because that is what was changed from the prev vert
        //(the vert is (k-1) long so each edge is k long)
        //000 and 001 share 00 so I can just output the 1
        //if it is the first then it will output what was 
        //changed from the last vert because we are in a cycle
        int last_bit = *it & 1;
        cout<<last_bit;
    }
}

vector<vector<int>> create_k_mer_graph(const int& k){
    //in the graph we have 2^(k-1) vertices
    int vert_count = (1 << (k-1));
    //each vert connects to (vert*2) % vert_count
    //and (vert*2 +1) % vert_count (the *2 shifts the binary by 1 bit to the left)
    //this is because we want the suffix of the source
    //to match the prefix of the target
    vector<vector<int>> graph(vert_count);
    for(int i = 0; i<vert_count; i++){
        int a = (i * 2) % vert_count;
        int b = (i * 2 + 1) % vert_count;
        graph[i].push_back(a);
        graph[i].push_back(b);
    }
    return graph;
...
int main(){
    int k;
    cin>>k;
    vector<vector<int>> graph = create_k_mer_graph(k);
    int vert_count = 1 << (k-1);
    int edge_count = vert_count * 2;
